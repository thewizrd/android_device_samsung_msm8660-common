...
if [ "$DATA_PARTITION" != "$VP_DATA_PARTITION" ]; then
  if [ "$SYSTEM_PARTITION" != "$VP_SYSTEM_PARTITION" ]; then
    ui_print "Partition layout isn't supported!"
    ui_print "This usually means that you are using a non-virtually partitioned recovery, such as one from http://twrp.me/."
    ui_print "In order to take full advantage of this ROM, you must flash my recovery."
    ui_print "My recovery makes it so everything flashes in the right place."
...
if [ "$PIT_MD5" != "$STOCK_PIT_MD5" ]; then
  ...
  ui_print "If I were to let you flash this ROM like this, you will run out of storage."
...
if [ "$DATA_FORMAT" != "ext4" ]; then
  if [ "$DATA_FORMAT" != "f2fs" ]; then
